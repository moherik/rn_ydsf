...
const AuthModal = ({navigation}) => {
  ...
  const signIn = async () => {
    setLoading(true);
    const confirmation = await auth().signInWithPhoneNumber(phone, true).finally(() => setLoading(false));
    ...
    <SafeAreaView style={{backgroundColor: '#fff', flex: 1}}>
      ...
      <ScrollView>
        <View style={styles.container}>
          ...
          {!verif ? (
            <Login signIn={signIn} setPhone={setPhone} setLoading={setLoading} />
          ) : (
            <Verif phone={phone} signIn={signIn} verif={verif} setVerif={setVerif} setPhone={setPhone} setLoading={setLoading} navigation={navigation} />
          )}
        </View>
      </ScrollView>
    </SafeAreaView>
